"""
test_sms_handler.py - Tests for SMS Handler (PY-001.11)
"""
import pytest
...
async def test_incoming_sms_triggers_llm(deps):
    ...
    await handler.handle_sms_event({"type": "sms_received", "payload": {"from": "+1", "body": "Hello!"}})
...
async def test_auto_reply_autonomous(deps):
    # UT-PY-001.11-02: Auto-reply matches autonomous
    config, bridge, ws_server, store, llm = deps
    config.voice_mode.default = "autonomous"
    handler = SMSHandler(config, bridge, ws_server, store, llm)
    
    await handler.handle_sms_event({"type": "sms_received", "payload": {"from": "+2", "body": "Ping"}})
...
async def test_draft_only_hitl(deps):
    # UT-PY-001.11-03: Draft only for HITL
    config, bridge, ws_server, store, llm = deps
    config.voice_mode.default = "hitl"
    handler = SMSHandler(config, bridge, ws_server, store, llm)
    
    await handler.handle_sms_event({"type": "sms_received", "payload": {"from": "+3", "body": "Test HITL"}})
    
    bridge.send_command.assert_not_called()
    ws_server.broadcast.assert_called_once_with({
        "type": "llm_sms_draft",
        "contact": "+3",
        "draft": "AI reply"
    })
...
async def test_conversation_history(deps):
    # UT-PY-001.11-04: Conversation history includes multiple messages
    config, bridge, ws_server, store, llm = deps
    ...
    handler = SMSHandler(config, bridge, ws_server, store, llm)
    await handler.handle_sms_event({"type": "sms_received", "payload": {"from": "+4", "body": "Msg 3"}})
